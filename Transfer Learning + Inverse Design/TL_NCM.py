...
#import data
data_import = pd.read_csv('./generated_data.csv')
...
path = "./NCM_model.h5"
checkpoint = ModelCheckpoint(path, monitor='val_loss', save_best_only=True, mode='min', verbose=1)
model.summary()
# fit model
...
with open("History.csv", "w", newline='') as file2:
...
with open("RESULT_NCM.csv", "w", newline='') as file3:
